...
const activeCount = computed(
    () => blocks.value.filter((item) => item.status).length,
);
...
function storeBlock() {
    ...
    if (editing.value.type === "list" || editing.value.type === "stats") {
        const items = (editing.value.config?.itemsString || "")
            .split("\n")
            .map((i) => i.trim())
            .filter(Boolean);
    ...
    if (editing.value.type === "card") {
        const cards = (editing.value.config?.cardsString || "")
            .split("\n")
            .map((i) => i.trim())
            .filter(Boolean);
    ...
    if (editing.value.id) {
        Inertia.patch(
            route("admin.blocks.update", editing.value.id),
            { ...payload, order: editing.value.order || 0 },
            {
                preserveState: true,
                onSuccess: (page) => {
...
<template>
    <div class="p-6">
        <div class="flex items-center justify-between mb-6">
            <div>
                ...
                <p class="text-sm text-slate-500">
                    Changes here control the public Welcome page layout.
                </p>
            </div>
            <button
                @click="openModal"
        ...
        <div class="grid gap-3 bg-slate-800 rounded-xl p-3">
            <div class="text-sm text-slate-100">
                Active blocks: {{ activeCount }} / {{ blocks.length }}
            </div>
            <template v-if="blocks.length">
                <div
                    v-for="(block, idx) in blocks"
                    :key="block.id"
                    class="flex items-center gap-3 p-3 rounded border border-slate-700 bg-slate-900"
                >
                    <div
                        class="w-2 h-8 rounded"
                        :style="{ background: blockColor(block.type) }"
                    ></div>
                    <div class="grow">
                    ...
                    <button
                        ...
                        :class="
                            block.status ? 'text-emerald-400' : 'text-slate-400'
                        "
                    >
                    ...
                    <button
                        ...
                        class="px-2 py-1 text-xs rounded border text-indigo-300"
                    >
                        Edit
                    </button>
                    <button
                        @click="deleteBlock(block)"
                        class="px-2 py-1 text-xs rounded border text-rose-400"
                    >
                        Delete
                    </button>
                    <button
                        v-if="idx > 0"
                        @click="reorderBlocks(idx, idx - 1)"
                        class="px-2 py-1 text-xs rounded border"
                    >
                        ↑
                    </button>
            ...
            <div v-else class="py-6 text-center text-sm text-slate-400">
                No blocks configured yet. Create one to power the client layout.
            </div>
        </div>
    </div>
    ...
    >
        <div
            class="bg-slate-900 border border-slate-700 rounded-xl p-5 w-full max-w-xl"
        >
            <h2 class="text-lg font-semibold mb-3">
                {{ editing.id ? "Edit block" : "Create block" }}
            </h2>
            ...
            <div class="mb-3">
                ...
                <input
                    ...
                    class="w-full rounded px-2 py-1 bg-slate-800 border border-slate-700"
                />
            </div>
            ...
            <div class="mb-3">
                ...
                <template v-if="editing.type === 'banner'">
                    <div class="space-y-2">
                        <input
                            v-model="editing.config.headline"
                            placeholder="Headline"
                            class="w-full rounded px-2 py-1 bg-slate-800 border border-slate-700"
                        />
                        <input
                            v-model="editing.config.subtitle"
                            placeholder="Subtitle"
                            class="w-full rounded px-2 py-1 bg-slate-800 border border-slate-700"
                        />
                        <input
                            v-model="editing.config.cta"
                ...
                <template
                    v-if="editing.type === 'list' || editing.type === 'stats'"
                >
                    <textarea
                        v-model="editing.config.itemsString"
                        rows="4"
                        class="w-full rounded px-2 py-1 bg-slate-800 border border-slate-700"
                ...
                <template v-if="editing.type === 'card'">
                    <textarea
                        v-model="editing.config.cardsString"
                        rows="4"
                        class="w-full rounded px-2 py-1 bg-slate-800 border border-slate-700"
                        placeholder="One card title per line"
                    ></textarea>
                </template>
            </div>
            ...
            <div class="flex justify-end gap-2">
                <button
                    @click="closeModal"
                    class="px-3 py-1 rounded border bg-slate-700"
                >
                    Cancel
                </button>
                <button
